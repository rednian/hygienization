<template>
    <div v-show="isOpened" class="radialMenu" :style="{width: size + 'px', height: size + 'px'}">
        ...
        <template v-for="(menu, index) in openedMenu">
            <svg
            :key="index"
        class="menu" 
            ...
            :id="'lvl'+ menu.level">
            <template v-for="(sector, index) in menu.sectors">
                <g 
                :key="index"
                @click="$_sectorClick"  
                    @mouseover="$_sectorHover" 
                    :class="{
                        sector: sector.id, dummy: !sector.id, 
                    ...
                    <text v-if="sector.title" text-anchor="middle" font-size="27%" :x="sector.centerX" :y="sector.centerY" :transform="sector.textTransform">{{sector.title}}</text>
